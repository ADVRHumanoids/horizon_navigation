...
MapTransformer::MapTransformer(double transformed_width, double transformed_height):
    _tfBuffer(),
    _tfListener(_tfBuffer),
    _transformed_width(transformed_width),
    _transformed_height(transformed_height)
{
...
//    _global_map_filtered_pub = _nh.advertise<nav_msgs::OccupancyGrid>("/global_map_filtered", 1);

    _transformed_map_pub = _nh.advertise<nav_msgs::OccupancyGrid>("/transformed_map", 1);

    auto global_map = ros::topic::waitForMessage<nav_msgs::OccupancyGrid>("/global_map", _nh);
    auto local_map = ros::topic::waitForMessage<nav_msgs::OccupancyGrid>("/local_map", _nh);

    _transformed_map.header.frame_id = "base_link";

    _transformed_width = 10.0;
    _transformed_height = 10.0;

    _transformed_map.info.origin.position.x = - _transformed_width / 2;
    _transformed_map.info.origin.position.y = - _transformed_height / 2;
    _transformed_map.info.origin.position.z = 0;
    _transformed_map.info.origin.orientation.x = 0;
    _transformed_map.info.origin.orientation.y = 0;
    _transformed_map.info.origin.orientation.z = 0;
    _transformed_map.info.origin.orientation.w = 1;

    _transformed_map.info.resolution = 0.015; //local_map->info.resolution;

    _transformed_map.info.width = static_cast<unsigned int>(_transformed_width / _transformed_map.info.resolution);
    _transformed_map.info.height = static_cast<unsigned int>(_transformed_height / _transformed_map.info.resolution);
    _transformed_map.data.resize(_transformed_map.info.width * _transformed_map.info.height, -1);


    _world_map.header.frame_id = "map";

    int world_map_width = 20.0;
    int world_map_height = 20.0;
    ...
    _world_map.info.resolution = 0.015; //local_map->info.resolution;
...
{
    if (_latest_global_map) {
        try
        {
            // Get the transformation from /map to /base_link
            geometry_msgs::TransformStamped base_link_T_map = _tfBuffer.lookupTransform("base_link", "map", ros::Time(0));
            geometry_msgs::TransformStamped map_T_base_link = _tfBuffer.lookupTransform("map", "base_link", ros::Time(0));
            geometry_msgs::TransformStamped odom_T_base_link = _tfBuffer.lookupTransform("base_link", "odom", ros::Time(0));
            geometry_msgs::TransformStamped identity_T;

            identity_T.transform.translation.x = 0.0;
            identity_T.transform.translation.y = 0.0;
            identity_T.transform.translation.z = 0.0;
            identity_T.transform.rotation.x = 0.0;
            identity_T.transform.rotation.y = 0.0;
            identity_T.transform.rotation.z = 0.0;
            identity_T.transform.rotation.w = 1.0;



            // update the world map only outside of exclusion zone
//            _transformed_map = bOverA(_transformed_map, *_latest_local_map, identity_T, 3.0, 3.0);
            // transform the local map from /base_link to /map frame and
            _world_map = transformFilter(_world_map, *_latest_local_map, base_link_T_map, 3.0, 3.0);

            // transform map in local frame
//            _local_from_global_map = bToA(_transformed_map, *_latest_global_map, map_T_base_link);

//            _local_from_global_map = filterGlobal(_local_from_global_map, _filtered_local_from_global_map, 2.0, 2.0);

            // filter map with the local map outside of the exlusion zone
//            _filtered_local_from_global_map = bOverA(_local_from_global_map, *_latest_local_map, identity_T, 3.0, 3.0);

            // outside the exlusion zone --> remove obstacles that are not in the current map

            _transformed_map_pub.publish(_world_map);
        }
...
nav_msgs::OccupancyGrid MapTransformer::transformFilter(nav_msgs::OccupancyGrid map_a,
